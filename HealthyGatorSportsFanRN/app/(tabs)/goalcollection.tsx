...
function confirmGoals(
  ...
  if (!feelBetter && !loseWeight) {
    Alert.alert('Goals missing', 'Make sure to select a goal!');
    ...
    currentUser.goalWeight = parseFloat(goalWeight as any);
...
function addNewUser(navigation: any, currentUser: any) {
  fetch(`${AppUrls.url}/user/`, {
    ...
    .then((response) => {
      if (!response.ok) throw new Error('Failed to save user account');
    ...
    .then((data) => {
      ...
      navigation.navigate('HomePage', { currentUser }); // 调试用跳转保留
    })
    .catch((error) => {
